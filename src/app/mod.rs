...
/// Fields marked `pub(super)` are those that `blit.rs` needs to read
/// or mutate from `rebuild_render_targets`. Visibility is scoped to
/// the `app` module tree only — nothing leaks to the rest of the crate.
#[allow(dead_code)]
struct AppState {
    window: Arc<Window>,
