...
- **Audio reactive** — multi-backend capture (cpal ALSA, parec for PulseAudio monitor sources, pw-cat for native PipeWire) with auto-selection. RMS level + 32-band FFT + beat tracker expose `level / pulse / bpm / bands[i]` to every shader. The beat tracker is a two-stage PLL: an unlocked flux-driven path that folds intervals into a canonical BPM window and enters a lock on low-variance passages, plus a locked grid-driven path where the visible beat fires from `last_beat + k*interval` and flux peaks only do phase correction (EMA nudge) + liveness checking. Result on live DJ playback: exactly one blink per kick, no doubles on hats/snares. The HUD BPM readout shows `128*` when locked and `~128` while still tracking. On HDA laptops the mic/line-in combo jack is watched via `evdev` and capture auto-swaps on plug/unplug (`audio_auto_swap`, default on; overridden by an explicit `audio_device`).
